...
import { updatePassword, updateProfile } from '../account'
...
export default function ProfileSettingsScreen({ user, workspaces, currentWorkspaceId, onProfileUpdated, onWorkspaceSelected }: {
  user: UserProfile
  ...
  return <main className="profile-settings-screen" aria-labelledby="profile-settings-title">
    ...
    <div className="profile-settings-grid">
      ...
      <section className="profile-settings-section" aria-labelledby="security-section-title"><div className="profile-settings-section-heading"><div><p className="eyebrow">Account &amp; Security</p><h2 id="security-section-title">Change Password</h2></div></div><p className="profile-section-intro">Update the password you use to sign in to WeekFlow.</p><form className="profile-settings-form" onSubmit={changePassword} noValidate><label className="profile-field"><span>New password</span><input type="password" value={newPassword} onChange={(event) => { setNewPassword(event.target.value); setPasswordStatus('idle'); setPasswordError('') }} autoComplete="new-password" /></label><label className="profile-field"><span>Confirm new password</span><input type="password" value={confirmPassword} onChange={(event) => { setConfirmPassword(event.target.value); setPasswordStatus('idle'); setPasswordError('') }} autoComplete="new-password" /></label>{passwordError && <p className="profile-message profile-error" role="alert">{passwordError}</p>}<div className="profile-action-row"><button className="button button-secondary" type="submit" disabled={passwordStatus === 'saving'}>{passwordStatus === 'saving' ? 'Saving...' : 'Change Password'}</button>{passwordStatus === 'saved' && <span className="profile-saved" role="status">Saved</span>}</div></form></section>

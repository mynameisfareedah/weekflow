        ...
        {TEMPLATE_GROUPS.map((group) => (
          <section className="template-group" key={group.title} aria-labelledby={`${group.title.toLowerCase().replace(/[^a-z]+/g, '-')}-heading`}>
            ...
            <div className="template-selection-grid">
              {group.ids.map((templateId, index) => {
                ...
                return (
                  <article className={`template-option${isSelected ? ' is-selected' : ''}${template.id === 'field-sales' ? ' is-featured' : ''}`} key={template.id}>
                    <div className="template-option-topline"><span className="template-option-mark" aria-hidden="true">{templateMarks[index]}</span><span className="template-option-status">{template.availability === 'available' ? 'Available' : 'Foundation ready'}</span></div>
                    {template.id === 'field-sales' && <span className="template-option-featured">Featured</span>}
                    <h2>{template.name}</h2>
                    <p>{template.description}</p>
                    <button className="button button-primary" type="button" onClick={() => chooseTemplate(template.id)}>{isSelected ? 'Selected' : 'Use this workflow'} <span aria-hidden="true">→</span></button>
                  </article>
                )
              })}
            </div>
